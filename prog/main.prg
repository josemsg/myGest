...
WITH _Screen
   ...
   *__ Añadir propiedades para nombre de usuario en activo
...
**** Agregar el objeto conexion a _SCREEN, este nos servirá en todo el
...
IF NOT lCancelar
	*__ Vamos a crear las conexiones con la BD de Telematel
	...
	IF lCancelar
		MESSAGEBOX("No se ha podido conectar con las bases de datos de Telematel",0+16,"Atención")
	ENDIF
ENDIF

IF lCancelar
	*__ NO hemos conectado con todas las bases de datos, así que salimos del programa
	lcCmd = LimpiarParaSalir()
	&lcCmd
ELSE
	*__ Hemos hecho login, ahora vamos a crear una conexión a telematel, en total 3
	...
	*__ Quedamos en el buche de la aplicaciión.
	if version(2)==0
  		read events
	ENDIF
