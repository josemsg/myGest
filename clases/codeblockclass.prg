...
* Revision 3.1(d), October 28, 1998
...
DIMENSION aCode[ 1]     && Array to store pre-processed code.
...
LOCAL lnOldMemo, lnOldMline, lcOldExact, lcRawLine, lcUpper, lcCleanLine, ;
	lnRawLines, ii, jj, ;
	lnDepth, lnCleanLines, lcRawCode, ;
...
lnDepth = 0
lcRawCode = THIS.cCodeBlock
lnRawLines = MEMLINES( m.lcRawCode )
DIMENSION THIS.aCode[ m.lnRawLines]
* Max possible, so we don't re-DIM all the time.
lcCleanLine = ""
lnCleanLines = 0

ii = 0
*FOR ii = 1 TO m.lnRawLines
DO WHILE .T.
	* Cannot use FOR..ENDFOR, since lnRawLines can grow,
		...
		* lnCleanLines = m.lnCleanLines + 1
		IF LTRIM( STRTRAN( UPPER( m.lcRawLine ), CHR( 9))) = "ENDT"
			* First, the accumulated block of text:
			lnCleanLines = m.lnCleanLines + 1
			THIS.aCode[ m.lnCleanLines] = m.lcTextBlock
			...
			* Then, the ENDTEXT:
			...
			IF m.lnCleanLines > m.lnRawLines  && unlikely
				DIMENSION THIS.aCode[ m.lnCleanLines]
			ENDIF
			
			THIS.aCode[ m.lnCleanLines] = "ENDTEXT"
	...
	* Strip out any TAB's in the code (simplifies later processing):
	...
	ELSE  && new line of code
		lcCleanLine = ALLTRIM( STRTRAN( ;
			m.lcRawLine, CHR(9), SPACE(1) ) )
	ENDIF
	IF EMPTY( m.lcCleanLine) OR ;
		INLIST( LTRIM( m.lcCleanLine), "*", "&" + "&")
	...
	* Note gymnastics to avoid compile error.
	...
	ELSE
		llComment = .F.
	ENDIF
	IF RIGHT( m.lcCleanLine, 1) = CHR( 59) AND m.ii < m.lnRawLines
		* Semi-colon.
		IF m.llComment
	...
	* Insert any #INCLUDE files:
	IF PADR( UPPER( m.lcCleanLine), 9) == "#INCLUDE "
		...
		IF FILE( m.lcStr)
			...
			lcRawCode = STUFF( m.lcRawCode, _MLINE + 1, 0, m.lcStr )
			lnRawLines = MEMLINES( m.lcRawCode )
			IF ALEN( THIS.aCode) < m.lnRawLines
				DIMENSION THIS.aCode[ m.lnRawLines ]
			ENDIF
			LOOP
		ELSE
	...
	* --- Main part of pre-processing. Create a "code diagram":
	...
	CASE PADR( m.lcUpper, 7) == "PRIVATE" AND ;
		...
		*
		IF m.lnCleanLines + 1 > m.lnRawLines && need 2 lines
			DIMENSION THIS.aCode[ m.lnCleanLines + 1]
		ENDIF
		* First the PRIVATE itself:
		THIS.aCode[ m.lnCleanLines] = m.lcCleanLine
		lnCleanLines = m.lnCleanLines + 1
...
CASE THIS.lMacroError = .T.  && typical case
	FOR ii = THIS.nErrorLine - m.tnLines ;
		...
		*
		lcLine = THIS.GetPreProcessedCodeBlock( m.ii, m.ii )
...
FOR _0_qnPointer = m._0_qnStart TO m._0_qnEnd
	...
	_0_qcLine = LTRIM( THIS.aCode[ m._0_qnPointer] )
	...
	CASE PADR( m._0_qcUpper, 4) == "TEXT"
		...
		* All of TEXT block is stored in 1 record/array element:
		_0_qcString = THIS.aCode[ m._0_qnChildEnd ]
		...
		* Is it an array?:
		...
		IF m._0_qlForArray
			...
			FOR _0_qnForPtr = 1 TO m._0_qnForLen
				...
				FOR _0_qnForLinePtr = m._0_qnChildStart TO m._0_qnChildEnd
					* Translate command:
					_0_qcForDoCmd = ForEachTran( THIS.aCode[ m._0_qnForLinePtr], ;
		...
		ELSE  && OLE collection
			...
			DO WHILE NOT THIS.lError
				...
				FOR _0_qnForLinePtr = m._0_qnChildStart TO m._0_qnChildEnd
					* Translate command:
					_0_qcForDoCmd = THIS.ForEachTran( THIS.aCode[ m._0_qnForLinePtr], ;
						m._0_qcForEach, m._0_qcForObj)
					* Run it:
					&_0_qcForDoCmd
					IF THIS.lError = .T. 
				...
				ELSE
					IF NOT m._0_qlFoundTrueCase  && no true case yet
						IF EVAL( SUBSTR( LTRIM( ;
							THIS.aCode[ THIS.aDiag[ m._0_qnDiagItem, 1] ] ), 5))  && TRUE case
							_0_qlInTrueCase = .T.
	...
	*-- Object reference to codeblock execution module.
	...
	PROCEDURE cmdverify.Click
		...
		ELSE
			...
			COPY MEMO Code TO (m.lcFile + ".PRG")
			USE IN (m.lcFile)
			COMPILE (m.lcFile + ".PRG")
			IF FILE(m.lcFile + ".ERR")
				MODIFY COMMAND (m.lcFile + ".ERR")
				ERASE (m.lcFile + ".PRG")
			ELSE
				= MessageBox( "No compilation errors.")
			ENDIF
			IF FILE(m.lcFile + ".PRG")
				ERASE (m.lcFile + ".PRG")
			ENDIF
			IF FILE(m.lcFile + ".FXP")
				ERASE (m.lcFile + ".FXP")
			ENDIF
			SELECT (m.lnSelect)
		ENDIF
